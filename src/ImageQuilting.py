...
class ImageQuilting:
    ...
    # Function to generate the matrix which contains the node numbers which are
    # used to in adjacency matrix to create weights
    ...
    # Function which creates the dict to get the position of a node
    # In the overlap matrix. It is useful while segmenting the image
    # Based on the graph cut.
    ...
    # Calculating error value for two patches only in the overlap region
    def leastOverlapError(self, random_sample_patch, samplePatchDimensions, overlap, y, x):
        error = 0
        # Calculating error of the patches merged sideways
        if x > 0:
            left = random_sample_patch[:, :overlap] - self.res[y:y + samplePatchDimensions, x:x + overlap]
            error = error + np.sum(left * left)

        # Calculating error of the patches merges from top and bottom
        if y > 0:
            up = random_sample_patch[:overlap, :] - self.res[y:y + overlap, x:x + samplePatchDimensions]
            error = error + np.sum(up * up)

        # Calculating the error for the overlap * overlap region formed when there is a
        # patch on the left and the top of the random sample patch
        # Subracting it because it is already calculated in twice in the above if conditions
        if x > 0 and y > 0:
            corner = random_sample_patch[:overlap, :overlap] - self.res[y:y + overlap, x:x + overlap]
            error = error - np.sum(corner * corner)

        # Returning the total error obtained
        return error
    ...
    def graphMinCutPatch(self, patch, patchLength, overlap, y, x, i, j):
        patch = patch.copy()
        ...
        if y == 0:  # First Row, simple vertical cuts
            ...
            self.res[:patchLength, x + overlap:x + patchLength] = patch[:, overlap:]
            ...
            # Filling The Non Overlapping part
            self.res[y + overlap:y + patchLength, x + overlap:x + patchLength] = patch[overlap:, overlap:]
            self.plotGraph()

        return patch

    def myQuiltFunction(self, image, samplePatchDimensions, noOfRows, noOfColumns):

        #Deriving the overlap size based on the samplePatch Dimensions
        ...
        #Creating a resultant matrix to hold the final quilted image
        ...
        for i in range(noOfRows):
            for j in range(noOfColumns):
                ...
                if i == 0 and j == 0:
                    ...
                    self.graphMinCutPatch(patch, samplePatchDimensions, overlap, y, x, i, j)

        return self.res

    def start(self, x, y, path, tileSize):

        #Reading the image and covnerting RGB pixels to float
        image = cv2.imread(path)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) / 255.0
        plt.imshow(image)
        plt.show()

        #Calling function which starts the quilting
        self.myQuiltFunction(image, tileSize, x, y)
        plt.imshow(self.res)
        plt.show()
...
if __name__ == "__main__":
    ...
    imageQuilting.start(x, y, image_path, tile_size)
